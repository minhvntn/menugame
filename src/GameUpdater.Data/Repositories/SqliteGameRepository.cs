...
public sealed class SqliteGameRepository : IGameRepository
{
    ...
    public async Task<IReadOnlyList<GameRecord>> GetAllAsync(CancellationToken cancellationToken = default)
    {
        ...
        command.CommandText =
            """
            SELECT Id, Name, Category, InstallPath, Version, LaunchRelativePath, LaunchArguments, LastScannedAt, LastUpdatedAt, Notes
            FROM Games
    ...
    public async Task<int> InsertAsync(GameRecord game, CancellationToken cancellationToken = default)
    {
        ...
        command.CommandText =
            """
            INSERT INTO Games (Name, Category, InstallPath, Version, LaunchRelativePath, LaunchArguments, LastScannedAt, LastUpdatedAt, Notes)
            VALUES ($name, $category, $installPath, $version, $launchRelativePath, $launchArguments, $lastScannedAt, $lastUpdatedAt, $notes);
